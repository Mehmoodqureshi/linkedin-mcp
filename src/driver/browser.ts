...
export interface BrowserManagerOptions {
  ...
  /** Override the userData root (mainly for tests). Defaults to `~/.linkedin-mcp`. */
  userDataDir?: string;
  /**
   * When set, drive the user's REAL Chrome profile of this name (e.g. "Default",
   * "Profile 1") instead of a dedicated automation profile — so their existing
   * login/extensions are used. Requires their normal Chrome to be CLOSED (Chrome
   * locks a profile to one process). See `chromeUserDataDir`.
   */
  chromeProfile?: string;
  /**
   * The Chrome user-data-dir that holds `chromeProfile`. Defaults to the OS
   * Google Chrome location. Ignored unless `chromeProfile` is set.
   */
...
/** Where the persistent Chrome profile + session artifacts live. */
...
/** The OS default Google Chrome user-data-dir (holds Default / Profile N). */
function systemChromeUserDataDir(): string {
...
export class BrowserManager extends EventEmitter {
  ...
  /** The dir passed to launchPersistentContext (dedicated profile, or real Chrome dir). */
  private readonly launchDir: string;
  /** `--profile-directory=<name>` when driving a real Chrome profile, else null. */
  private readonly profileArg: string | null;
  /** True when we own the profile (safe to reclaim its lock); false for the user's real Chrome. */
  private readonly ownsProfile: boolean;
  ...
  constructor(options: BrowserManagerOptions = {}) {
    ...
    if (options.chromeProfile) {
      // Drive the user's REAL Chrome profile: point --user-data-dir at their
      // Chrome dir and select the named profile. We do NOT own this profile, so
      // we must never reclaim its lock by killing the owner (that's their Chrome).
      this.launchDir = options.chromeUserDataDir ?? systemChromeUserDataDir();
      this.profileArg = `--profile-directory=${options.chromeProfile}`;
      this.ownsProfile = false;
    } else {
      this.launchDir = this.profileDir;
      this.profileArg = null;
  ...
  private async launchWithLockRecovery(
    ...
    try {
      return await chromium.launchPersistentContext(this.launchDir, contextOptions);
    } catch (err) {
      ...
      // Driving the user's REAL Chrome profile: the lock owner is their own
      // running Chrome. NEVER kill it — tell them to quit Chrome and retry.
      if (!this.ownsProfile) {
        throw new Error(
          'Google Chrome is already running with this profile, so it cannot be ' +
            'driven at the same time. Quit Chrome completely, then retry.',
        );
      }

      const owner = inspectProfileLock(this.launchDir);
      ...
      clearProfileLocks(this.launchDir);
      process.stderr.write(
  ...
  private async doLaunch(): Promise<BrowserContext> {
    ...
    // Driving the user's real Chrome profile: select it explicitly.
    if (this.profileArg) args.push(this.profileArg);

    // Standalone mode: the window is VISIBLE (no Electron app mirrors it), so the
    // user can log in and watch actions. We deliberately do NOT push it off-screen
    // — a genuine headed browser is also what third-party identity providers
    // (e.g. Google sign-in) accept where they reject headless.

    const contextOptions: PersistentContextOptions = {
      headless: this.headless,
      slowMo: this.slowMo,
      // Use the user's INSTALLED Google Chrome, not a Playwright-downloaded
      // browser — so there is no ~120MB browser download at install time.
      channel: 'chrome',
    ...
    try {
      ...
      throw new Error(
        `Failed to launch Chrome at ${this.launchDir}: ${(err as Error).message}`,
      );
    }
